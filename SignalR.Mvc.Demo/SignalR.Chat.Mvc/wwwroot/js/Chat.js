    ...
    if (connection === null) {
        ...
        connection.on("Broadcast", function (response) {
            $("#chatRoom").append(
                ...
                '<p class="small text-muted">' + response.timestamp + '</p>' +
                '</div>' +
                '</div>'
            );
        ...
        connection.on("UserConnected", function (response) {
            $("#chatUsers").append(
                ...
                '<h6 class="mb-0">Some User</h6><small class="small font-weight-bold">' + response.timeStamp + '</small>' +
                '</div>' +
                '<p class="font-italic mb-0 text-small">' + response.connectedId + '</p>' +
                '</div>' +
                '</div>' +
                '</a>'
            );
            ...
            $("#chatRoom").append(
                ...
                '</div>' +
                '<p class="small text-muted">' + response.messageTimeStamp  + '</p>' +
                '</div>'
            );
        });
